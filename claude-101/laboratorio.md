...
Antes de abrir cualquier herramienta, pónganse de acuerdo en una frase: *"nuestro producto le permite a \_\_\_ hacer \_\_\_ para que \_\_\_"*. Si todavía no la tienen, ahí está el primer frente de trabajo.
...
1.  Afilar el problema y el usuario
    ...
    **Qué pedir:** que actúe como product researcher, que a partir de su descripción liste usuarios candidatos, hipótesis y riesgos, y que marque explícitamente qué está asumiendo.
    ...
    **Cómo saben que quedó:** pueden nombrar una persona concreta, con qué frecuencia le pasa el problema y qué usa hoy en su lugar. Y tienen escrita al menos una hipótesis que podría resultar falsa.
    ...
    Un modelo genera hipótesis; validarlas es otra cosa y se hace hablando con usuarios.

2.  Recortar el MVP

    **Les sirve si** su lista de funcionalidades tiene más de cinco cosas, o si no hay nada explícitamente fuera de alcance.

    **Qué pedir:** que ordene las funcionalidades por valor para el usuario contra esfuerzo, que proponga el corte mínimo que ya resuelve el problema, y que justifique cada exclusión.
...
3.  User stories y criterios de aceptación
    ...
    **Les sirve si** van a empezar a programar. Sin criterios de aceptación no pueden evaluar lo que salga generado.
...
4.  Diseño técnico
    ...
    **Qué pedir:** que proponga dos o tres arquitecturas alternativas para su caso con sus contras, no una sola. Ustedes eligen y escriben por qué.

    **Cómo saben que quedó:** tienen el stack cerrado, las entidades principales y los endpoints o pantallas del MVP. Y una decisión que tomaron *en contra* de lo que propuso el modelo, con su razón.
...
5.  Construir con un agente de código
    ...
    **El orden que funciona:** escribir un archivo de instrucciones del repo antes del primer prompt, trabajar en rama, pedir cambios pequeños, leer el diff, correr los tests, hacer commit. Repetir.

    Ese archivo se llama `CLAUDE.md` en Claude Code; otros agentes leen su equivalente. El contenido es el mismo:

    

    Archivo de instrucciones mínimo para su repo
...
Es la parte que más pesa de la evaluación, porque es donde se ve el criterio del equipo.
...
Se responde en un párrafo al final.
...
| Diseño técnico          | Es lo que salió del primer prompt, sin comparar alternativas | Stack y entidades justificados             | Se compararon alternativas y se documentó la decisión                    |
...
| Comprensión             | El equipo no puede explicar el código entregado              | El equipo explica lo esencial              | Cualquiera explica cualquier parte y la modifica en vivo                 |

### Tres niveles, según dónde esté el equipo

Nivel 1 — Prompting

Trabajan con un modelo en el chat, o en un Project si usan Claude.
...
**Reto:** un archivo de instrucciones propio, una feature implementada en rama con su test de caso borde, y el diff revisado con comentarios antes del merge.
...
| Un agente de código                 | Claude Code si tienen plan pago; si no, cualquiera de las opciones gratuitas de la guía de herramientas |
| Carpeta de evidencia                | `ai-evidence/` en el repositorio, desde el primer commit                                                |
| Si nadie del equipo tiene plan pago | Hay opciones gratuitas que sirven igual para este taller. Ver la guía de herramientas.                  |

La biblioteca de prompts tiene 26 plantillas por rol. Funcionan con cualquier modelo: empiecen por la del frente en el que están y reemplacen los marcadores con los datos de su producto.
